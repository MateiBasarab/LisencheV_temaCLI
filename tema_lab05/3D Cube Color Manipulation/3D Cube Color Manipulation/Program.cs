...
namespace _3D_Cube_Color_Manipulation_
{
    public class Game : GameWindow
    {
        ...
        private void EnsureFileExists(string fileName)
        {
            ...
            {
                Console.WriteLine("Fisierul nu există. \nCream fișierul cu date implicite...");
                string[] defaultData = {
        ...
        protected override void OnUpdateFrame(FrameEventArgs e)
        {
            ...
            if (keyboardState.IsKeyDown(Key.Escape)) Exit(); // Iesire program

                if (keyboardState.IsKeyDown(Key.R))
                ChangeCubeColor(0, new Color4(1f, 0f, 0f, 1f)); // Cub rosu

            if (keyboardState.IsKeyDown(Key.G))
                ChangeCubeColor(0, new Color4(0f, 1f, 0f, 1f)); // Cub verde

            if (keyboardState.IsKeyDown(Key.B))
                ChangeCubeColor(0, new Color4(0f, 0f, 1f, 1f)); // Cub albastru

            if (keyboardState.IsKeyDown(Key.V))
                ChangeCubeColor(0, GetRandomColor()); // Culori Random

            if (keyboardState.IsKeyDown(Key.T))
                ChangeCubeColor(0, new Color4(1f, 0f, 0f, 0.5f)); // Cub rosu semi-transparent (neobservabil)

            if (keyboardState.IsKeyDown(Key.C)) // Revenire culori initiale
                ResetCubeColor(0);
